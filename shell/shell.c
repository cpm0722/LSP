...
char **tokenize(char *line)
{
	...
	char *nowToken = (char *)malloc(MAX_TOKEN_SIZE * sizeof(char));
	int tokenNo = 0;	//tokens에서의 idx (현재 몇번째 token인가)
	int tokenIdx = 0;	//tokens[tokenNo]에서의 idx
	...
	for(int i =0; i < strlen(line); i++){	//line 순회
		...
		if (ch == ' ' || ch == '\n' || ch == '\t' || ch == '\0'){	//공백문자일 경우 tokenize
			if (tokenIdx != 0){
				nowToken[tokenIdx] = '\0';	//현재 token 마지막 null문자 추가
				tokens[tokenNo] = (char*)malloc(MAX_TOKEN_SIZE*sizeof(char));	//다음 token 동적 할당
				if(!strcmp("pps", nowToken) || !strcmp("ttop", nowToken)){	//상대 경로로 변경
					strcpy(tokens[tokenNo], "./");
					strcat(tokens[tokenNo++], nowToken);	//현재 tokens[tokenNo]에 복사
				}
				else
					strcpy(tokens[tokenNo++], nowToken);	//현재 tokens[tokenNo]에 복사
				tokenIdx = 0; 
			}
		}
		else {
			nowToken[tokenIdx++] = ch;
		}
	}
	free(nowToken);
	tokens[tokenNo] = NULL;
...
// 파이프 처리하는 함수
void do_pipe(char **tokens, int pipeIdx[TOKEN_CNT], int pipeCnt)
{
	...
	if((pid = fork()) < 0){
		...
		execvp(tokens[pipeIdx[0]], &tokens[pipeIdx[0]]);	//exec
	...
	for(int i = 0; i < pipeCnt - 1; i++){
		...
		if((pid = fork()) < 0){
			...
			execvp(tokens[pipeIdx[i+1]], &tokens[pipeIdx[i+1]]);	//exec
			fprintf(stderr, "execvp() error\n");
		}
		close(pipes[i+1][1]);		//pipe close
		wait(&status);				//exec 종료까지 대기
	...
	if((pid = fork()) < 0){
		...
		close(STDIN_FILENO);		//stdin close
		dup2(pipes[pipeCnt-1][0], STDIN_FILENO); 	//pipe stdin에 복사
		close(pipes[pipeCnt-1][0]);	//pipe close
		close(pipes[pipeCnt-1][1]);	//pipe close

		execvp(tokens[pipeIdx[pipeCnt]], &tokens[pipeIdx[pipeCnt]]);	//exec
	}
...
void run_commands(char **tokens)
{
	...
	int pipeIdx[TOKEN_CNT] = {0, };

	for(int i = 0; tokens[i] != NULL; i++, len++){	//파이프 있을 경우 token NULL로 변경
		if(!strcmp(tokens[i], "|")){
			tokens[i] = NULL;
			pipeIdx[++pipeCnt] = i+1;		//pipdIdx에 pipe로 구분되는 token들 시작 index 저장
			hasPipe = true;
		}
	}

	if(hasPipe)		//파이프 있을 경우
		do_pipe(tokens, pipeIdx, pipeCnt);
	else{			//파이프 없을 경우
